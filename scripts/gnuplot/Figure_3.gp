...
set ylabel "Proportion of admissible combinatorial profiles (%)" offset -1.35,0
unset xlabel
set grid ytics

set label 100 "Fluid biospecimens" at first 3, first 0.585 center font "Helvetica,10"
set label 101 "Solid biospecimens" at first 9, first 0.585 center font "Helvetica,10"
...
plot DATA every ::1 using (stringcolumn("matrix") eq "fluid" ? xpos(stringcolumn("matrix"),stringcolumn("final_grade")) : 1/0):(column("proportion")) \
     ...
     DATA every ::1 using (xpos(stringcolumn("matrix"),stringcolumn("final_grade"))):(column("proportion") + 0.022):(sprintf("%.1f",100.0*column("proportion"))."\nn=".sprintf("%d",int(column("n")))) \
