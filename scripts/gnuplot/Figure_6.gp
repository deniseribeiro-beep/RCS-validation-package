...
set logscale x 2
set xrange [0.8:20]
set xtics ("1" 1, "2" 2, "4" 4, "8" 8, "16" 16) font sprintf("Helvetica,%.1f", FS_SMALL)
set yrange [0.2:3.1]
set ytics 0.5
set format y "%.1f"
set grid ytics
set arrow 900 from graph 0, first 1 to graph 1, first 1 nohead dashtype 3 linewidth (0.9*DEVICE_SCALE) linecolor rgb "#777777" back
...
set label 100 "(a) R / PSOCK" at graph 0.015,0.89 left font sprintf("Helvetica,%.1f", FS_PANEL)
...
set ylabel "Speedup (×)" offset 0.7,0
set label 101 "(b) Cython / OpenMP" at graph 0.015,0.89 left font sprintf("Helvetica,%.1f", FS_PANEL)
...
set xlabel "Threads" offset 0,0.25
set ylabel "Speedup (×)" offset 0.7,0
set label 102 "(c) C++ / OpenMP" at graph 0.015,0.89 left font sprintf("Helvetica,%.1f", FS_PANEL)
