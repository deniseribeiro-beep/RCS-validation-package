#ifndef RCS_BENCHMARK_COMMON_HPP
#define RCS_BENCHMARK_COMMON_HPP
...
inline void print_result(const Arguments& args, std::size_t n, int loops, double seconds,
                         ...
                         int attempts = 0) {
  std::cout << std::setprecision(12) << "RCSRESULT," << args.implementation << ',' << n << ','
            << args.threads << ',' << loops << ',';
...
inline void print_phases(double read_seconds, double init_seconds, double compute_seconds,
  ...
  std::cout << std::setprecision(12) << "RCSPHASES," << read_seconds << ',' << init_seconds << ','
